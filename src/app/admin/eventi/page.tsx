...
export default function EventiPage() {
  ...
  const fetchEventi = useCallback(async () => {
    setLoading(true);
    try { const res = await fetch("/api/eventi"); if (res.ok) setEventi(await res.json()); } catch {}
    setLoading(false);
  }, []);

  useEffect(() => { fetchEventi(); }, [fetchEventi]);

  function openAdd() { setEditId(null); setForm({ nome: "", data: new Date().toISOString().slice(0,10), oraInizio: "", oraFine: "", sedeId: sedi[0]?.id||0, sedeAltro: "", descrizione: "", sedeType: "sede" }); setShowForm(true); }
  function openEdit(e: Evento) { setEditId(e.id); setForm({ nome: e.nome, data: e.data, oraInizio: e.oraInizio||"", oraFine: e.oraFine||"", sedeId: e.sedeId||0, sedeAltro: e.sedeAltro||"", descrizione: "", sedeType: e.sedeId ? "sede" : "altro" }); setShowForm(true); }
              ...
              <div key={e.id} className={`rounded-xl border p-4 transition-colors ${e.data === oggi ? "border-accent/40 bg-accent/[0.03]" : passato ? "border-border bg-card-bg opacity-60" : `${sedeBorder(e.sede)} bg-card-bg`}`}>
                ...
                <div className="mt-3 flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => deleteEvento(e.id)} className="rounded-lg border border-red-500/30 px-3 py-1.5 text-xs text-red-400 hover:bg-red-500/10">Elimina</button>
                  </div>
                </div>
              </div>
            );
